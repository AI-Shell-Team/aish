...
impl SessionStore {
        ...
        if let Some(parent) = db_path.parent() {
            std::fs::create_dir_all(parent)
                .map_err(|e| AishError::Session(
                    format!("failed to create session db directory: {e}")
                ))?;
        }

        let conn = rusqlite::Connection::open(&db_path)
            .map_err(|e| AishError::Session(
                format!("failed to open session db at {:?}: {e}", db_path)
            ))?;
    ...
    pub fn get_session(&self, uuid: &str) -> Result<Option<SessionRecord>> {
        let mut stmt = self.conn.prepare(
            ...
            format!("failed to prepare get_session: {e}")
        ))?;
    ...
    pub fn list_sessions(&self, limit: usize) -> Result<Vec<SessionRecord>> {
        let mut stmt = self.conn.prepare(
            ...
            format!("failed to prepare list_sessions: {e}")
        ))?;
        ...
        for row in rows {
            sessions.push(row.map_err(|e| AishError::Session(
    ...
    pub fn get_history(&self, session_uuid: &str, limit: usize) -> Result<Vec<HistoryEntry>> {
        let mut stmt = self.conn.prepare(
            ...
            format!("failed to prepare get_history: {e}")
        ))?;

        let rows = stmt.query_map(params![session_uuid, limit], |row| {
            Ok(HistoryEntry {
                id: row.get(0)?,
        ...
        for row in rows {
            entries.push(row.map_err(|e| AishError::Session(
    ...
    pub fn close(self) -> Result<()> {
        self.conn
            .close()
            .map_err(|(_, e)| AishError::Session(
                format!("failed to close session db: {e}")
            ))
    }
}
