...
#[derive(Parser)]
#[command(name = "aish", version, about = "AI Shell - A shell with built-in LLM capabilities")]
struct Cli {
    /// LLM model to use
    #[arg(long, short = 'm')]
    model: Option<String>,
...
fn main() {
    ...
    match cli.command.unwrap_or(Commands::Run) {
        ...
        Commands::Update { check_only, pre_release } => {
...
fn run_setup(config: &mut aish_config::ConfigModel) {
    ...
    match wizard.run() {
        ...
        Err(e) => {
            eprintln!("\n\x1b[33m{}: {}\x1b[0m", aish_i18n::t("cli.setup.cancelled"), e);
...
fn check_langfuse(
    ...
    match (public_key, secret_key) {
        (Some(pk), Some(sk)) => {
            let lf_config = aish_llm::LangfuseConfig::from_parts(
                Some(&pk),
                Some(&sk),
                host.as_deref(),
            );
            match lf_config {
                Some(cfg) => {
                    ...
                    if pk.len() > 8 {
                        ...
                        println!("  Public Key: {}...{}", &pk[..2.min(pk.len())], &pk[(pk.len()-2).min(pk.len())..]);
                    }
