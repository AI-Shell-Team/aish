...
use crate::types::{PlanModeState, PlanPhase, PlanApprovalStatus};
...
pub const AWAITING_APPROVAL_VISIBLE_TOOLS: &[&str] = &[
    "read_file",
    "ask_user",
    "exit_plan_mode",
];
...
pub fn get_available_templates() -> Vec<PlanTemplate> {
    vec![
        PlanTemplate {
            ...
            description: "Standard plan template with overview, context, steps, and testing sections.".to_string(),
        ...
        PlanTemplate {
            ...
            description: "Feature template: Requirements, Design, Implementation, Testing.".to_string(),
...
pub fn ensure_plan_artifact(state: &mut PlanModeState) -> Result<(), std::io::Error> {
    ...
    let plan_id = state.plan_id.as_ref().ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::InvalidInput, "Plan ID not set")
    })?;
...
pub fn create_approved_snapshot(state: &mut PlanModeState) -> Result<PathBuf, std::io::Error> {
    let artifact_path = state.artifact_path.as_ref().ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::InvalidInput, "No artifact path")
    })?;

    let hash = compute_artifact_hash(Path::new(artifact_path))
        .ok_or_else(|| {
