...
use aish_security::{SecurityManager, SecurityDecision, SandboxIpc, PolicyRule};
use aish_security::sandbox_ipc::{SandboxRequest, SandboxResponse, FileChange};
...
fn test_security_manager_default_policy() {
    ...
    assert!(manager.policy().enable_sandbox == false || manager.policy().enable_sandbox == true);
}
...
fn test_check_command_confirms_risky_patterns() {
    ...
    match &decision {
        SecurityDecision::Confirm { reason } | SecurityDecision::Block { reason } => {
            assert!(reason.contains("rm -rf") || reason.contains("recursive") || reason.contains("delete"),
                    "Expected rm -rf related reason, got: {}", reason);
        }
    ...
    match &decision {
        SecurityDecision::Confirm { reason } | SecurityDecision::Block { reason } => {
            assert!(reason.contains("mkfs") || reason.contains("filesystem") || reason.contains("format"),
                    "Expected mkfs-related reason, got: {}", reason);
        }
        SecurityDecision::Allow => {
    ...
    match &decision {
        SecurityDecision::Confirm { reason } | SecurityDecision::Block { reason } => {
            assert!(reason.contains("dd") || reason.contains("disk"),
                    "Expected dd-related reason, got: {}", reason);
        }
        SecurityDecision::Allow => {
            // With default policy, might be allowed if no rule matches
        }
    ...
    match &decision {
        SecurityDecision::Confirm { reason } | SecurityDecision::Block { reason } => {
            assert!(reason.contains("chmod") || reason.contains("777") || reason.contains("writable"),
                    "Expected chmod-related reason, got: {}", reason);
        }
        SecurityDecision::Allow => {
    ...
    match &decision {
        SecurityDecision::Confirm { reason } | SecurityDecision::Block { reason } => {
            assert!(reason.contains("/etc/") || reason.contains("truncat") || reason.contains("system"),
                    "Expected /etc/ related reason, got: {}", reason);
        }
        SecurityDecision::Allow => {
            // With default policy, might be allowed if no rule matches
        }
...
fn test_sandbox_request_serialization() {
    ...
    assert_eq!(deserialized.readonly, true);
...
fn test_sandbox_response_serialization() {
    ...
    assert_eq!(deserialized.blocked, false);
}
...
fn test_security_manager_policy_access() {
    ...
    assert_eq!(retrieved_policy.sandbox_off_action, policy.sandbox_off_action);
}
...
fn test_check_command_case_insensitive() {
    ...
    assert!(matches!(manager.check_command("RM -RF /"), SecurityDecision::Block { .. }));
    assert!(matches!(manager.check_command("Rm -Rf /"), SecurityDecision::Block { .. }));
    assert!(matches!(manager.check_command("DD IF=/dev/zero OF=/dev/sda"), SecurityDecision::Confirm { .. }));
}
