...
impl SandboxDaemon {
    ...
    pub fn serve(&self) -> Result<(), String> {
        ...
        let listener = UnixListener::bind(&self.config.socket_path).map_err(|e| {
            format!("failed to bind socket at {}: {}", self.config.socket_path.display(), e)
        })?;
        ...
        while !self.stop_requested.load(std::sync::atomic::Ordering::Relaxed) {
...
fn execute_with_bwrap(
    ...
    let output = cmd.output().map_err(|e| format!("bwrap execution failed: {}", e))?;
