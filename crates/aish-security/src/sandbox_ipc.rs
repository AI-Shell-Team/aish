...
impl SandboxIpc {
    ...
    pub async fn execute(
        ...
        let conn = tokio::time::timeout(self.timeout, UnixStream::connect(&socket_path))
            .await
            .map_err(|_| format!("timeout connecting to sandbox socket after {:?}", self.timeout))?
...
mod tests {
    ...
    fn test_timeout_configuration() {
        let ipc = SandboxIpc::new("/tmp/test.sock").with_timeout(std::time::Duration::from_secs(60));
