...
impl SecurityManager {
    ...
    pub async fn check_command_async(&self, command: &str) -> SecurityDecision {
        ...
        for rule in &self.policy.rules {
            if let Some(ref cmds) = rule.command_list {
                ...
                for cmd_pattern in cmds {
                    if basename == cmd_pattern
                        || cmd_lower.contains(&cmd_pattern.to_lowercase())
                    {
                        let reason = rule
                            .reason
                            .clone()
                                        ...
                                        match rule.risk {
                                            aish_core::RiskLevel::High => {
                                                return SecurityDecision::Block {
                                                    reason: format!(
                                                        "{}: {} {}",
                                                        change.operation, path, rule.reason.as_ref().unwrap_or(&pattern.clone())
                                                    ),
                                                };
                                            }
                                            aish_core::RiskLevel::Medium => {
        ...
        let confirm_patterns = [
            ...
            ("chmod -R 777", "recursively making everything world-writable"),
