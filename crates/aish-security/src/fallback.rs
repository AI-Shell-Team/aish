...
impl FallbackRuleEngine {
    pub fn new() -> Self {
        Self {
            delete_commands: vec![
                "rm", "rmdir", "unlink", "truncate", "shred", "mv",
            ],
        }
    }
    ...
    fn parse_delete_command(&self, command: &str) -> Option<ParsedDeleteCommand> {
        ...
        let command_name = tokens[0].rsplit('/').next().unwrap_or(tokens[0]).to_string();

        // Check if it's a recognized delete command
        if !self
            .delete_commands
            .iter()
            .any(|dc| *dc == command_name)
        {
            return None;
        }
...
mod tests {
    ...
    fn test_parse_rm_multiple_paths() {
        let engine = FallbackRuleEngine::new();
        let parsed = engine.parse_delete_command("rm -f /etc/passwd /etc/shadow").unwrap();
        assert_eq!(parsed.command_name, "rm");
    ...
    fn test_parse_sudo_rm() {
        let engine = FallbackRuleEngine::new();
        let parsed = engine.parse_delete_command("sudo rm -rf /etc/important").unwrap();
        assert_eq!(parsed.command_name, "rm");
    ...
    fn test_parse_bash_c_wrapper() {
        let engine = FallbackRuleEngine::new();
        let parsed = engine.parse_delete_command("bash -c \"rm -rf /data/backup\"").unwrap();
        assert_eq!(parsed.command_name, "rm");
