...
use nix::unistd::{close, dup2, execvp, fork, ForkResult, Pid, pipe};
...
impl PersistentPty {
    ...
    pub fn start(cwd: &str, rows: u16, cols: u16) -> aish_core::Result<Self> {
        ...
        let (control_read, control_write) = pipe().map_err(|e| {
            AishError::Pty(format!("failed to create control pipe: {e}"))
        })?;

        // Create PTY.
        let pty_result = openpty(None, None).map_err(|e| {
            AishError::Pty(format!("failed to openpty: {e}"))
        })?;
        ...
        // Fork.
        let child_pid = match unsafe { fork() }.map_err(|e| {
            AishError::Pty(format!("fork failed: {e}"))
        })? {
    ...
    pub fn execute_command(
        ...
        let raw_output = self.exec_buffer.lock().unwrap().drain(..).collect::<Vec<u8>>();
    ...
    pub fn send_command_interactive(
        ...
        self.command_state.register_command(command, CommandSource::User, None);
        ...
        while !done {
            ...
            let mut tv = libc::timeval { tv_sec: 0, tv_usec: 50_000 }; // 50ms

            let sel = unsafe {
                libc::select(max_fd, &mut read_fds, &mut write_fds, std::ptr::null_mut(), &mut tv)
            };
            ...
            if unsafe { libc::FD_ISSET(self.master_fd, &write_fds) } && !write_buf.is_empty() {
                match unsafe {
                    libc::write(self.master_fd, write_buf.as_ptr() as *const libc::c_void, write_buf.len())
                } {
                    n if n > 0 => { write_buf.drain(..n as usize); }
                    _ => { write_buf.clear(); }
                }
            }

            // Read stdin -> master.
            if unsafe { libc::FD_ISSET(stdin_fd, &mut read_fds) } {
                let mut tmp = [0u8; 1024];
                match unsafe { libc::read(stdin_fd, tmp.as_mut_ptr() as *mut libc::c_void, tmp.len()) } {
                    n if n > 0 => {
            ...
            // Read master -> stdout.
            if unsafe { libc::FD_ISSET(self.master_fd, &mut read_fds) } {
                let mut tmp = [0u8; 8192];
                match unsafe { libc::read(self.master_fd, tmp.as_mut_ptr() as *mut libc::c_void, tmp.len()) } {
                    n if n > 0 => {
                        let data = &tmp[..n as usize];
            ...
            if unsafe { libc::FD_ISSET(self.control_fd, &mut read_fds) } {
                let mut tmp = [0u8; 4096];
                match unsafe { libc::read(self.control_fd, tmp.as_mut_ptr() as *mut libc::c_void, tmp.len()) } {
                    n if n > 0 => {
    ...
    fn drain_master_to_stdout(&self) {
        ...
        loop {
            match unsafe {
                libc::read(self.master_fd, tmp.as_mut_ptr() as *mut libc::c_void, tmp.len())
            } {
                n if n > 0 => {
                    let _ = unsafe {
                        libc::write(libc::STDOUT_FILENO, tmp[..n as usize].as_ptr() as *const libc::c_void, n as usize)
                    };
                }
            ...
            match unsafe {
                ...
                n if n > 0 => {
                    if self.exec_mode.load(Ordering::SeqCst) {
                        self.exec_buffer.lock().unwrap().extend_from_slice(&tmp[..n as usize]);
                    }
                }
                _ => break,
            }
    ...
    fn drain_control_pipe(&mut self) {
        ...
        loop {
            match unsafe {
                libc::read(self.control_fd, tmp.as_mut_ptr() as *mut libc::c_void, tmp.len())
            } {
                n if n > 0 => {
                    let events = decode_control_chunk(
    ...
    fn wait_for_session_ready(&mut self, timeout: Duration) -> aish_core::Result<()> {
        ...
        while std::time::Instant::now() < deadline {
            ...
            match unsafe { libc::read(self.control_fd, ctrl_tmp.as_mut_ptr() as *mut libc::c_void, ctrl_tmp.len()) } {
                n if n > 0 => {
                    let events = decode_control_chunk(
                        &mut self.control_buffer,
                        &ctrl_tmp[..n as usize],
                ...
                0 => {
                    ...
                    self.running.store(false, Ordering::SeqCst);
                    return Err(AishError::Pty("control pipe closed before session_ready".into()));
                }
                _ => {}
            }
        ...
        Err(AishError::Pty("timeout waiting for session_ready event".into()))
    }

    fn wait_for_prompt_ready(&mut self, expected_seq: i32, timeout: Duration) -> Option<crate::types::PtyCommandResult> {
        let deadline = std::time::Instant::now() + timeout;
        while std::time::Instant::now() < deadline {
...
fn set_nonblocking(fd: &OwnedFd) -> aish_core::Result<()> {
    ...
    let flags = fcntl(raw, FcntlArg::F_GETFL).map_err(|e| {
        AishError::Pty(format!("fcntl F_GETFL failed: {e}"))
    })?;
    let flags = OFlag::from_bits_truncate(flags) | OFlag::O_NONBLOCK;
    fcntl(raw, FcntlArg::F_SETFL(flags)).map_err(|e| {
        AishError::Pty(format!("fcntl F_SETFL O_NONBLOCK failed: {e}"))
    })?;
    Ok(())
}
...
fn sync_window_size(src_fd: RawFd, dst_fd: RawFd) -> nix::Result<()> {
    ...
    if rc >= 0 {
        unsafe { libc::ioctl(dst_fd, libc::TIOCSWINSZ, &ws); }
    }
    Ok(())
}
...
fn write_rcfile_temp() -> aish_core::Result<std::path::PathBuf> {
    ...
    std::fs::write(&path, BASH_RC_WRAPPER).map_err(|e| {
        AishError::Pty(format!("failed to write rcfile temp: {e}"))
    })?;
    Ok(path)
}
