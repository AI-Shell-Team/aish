...
impl CommandSubmission {
    pub fn new(command: String, source: CommandSource, command_seq: Option<i32>) -> Self {
        let allow_error_correction = source == CommandSource::User
            && !Self::is_interactive_session_command(&command);
        Self {
            command,
            source,
            command_seq,
...
mod tests {
    ...
    fn test_is_interactive_session_command() {
        assert!(CommandSubmission::is_interactive_session_command("ssh user@host"));
        assert!(CommandSubmission::is_interactive_session_command("telnet example.com"));
        assert!(!CommandSubmission::is_interactive_session_command("ls -la"));
        assert!(!CommandSubmission::is_interactive_session_command("vim file.txt"));
    }
}
