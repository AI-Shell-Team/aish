...
impl PtyOutputOffload {
    ...
    fn ensure_overflow_file(&mut self, stream: StreamName) -> Result<(), AishError> {
        ...
        if !has_overflow {
            ...
            let dir = self.ensure_offload_dir().map_err(|e| {
                AishError::Pty(format!("failed to create offload dir: {e}"))
            })?;
            ...
            fs::File::create(&path).map_err(|e| {
                AishError::Pty(format!("failed to create overflow file: {e}"))
            })?;
...
mod tests {
    ...
    fn test_permissions_0600() {
        ...
        if offload_dir.exists() {
            ...
            assert_eq!(mode & 0o777, 0o700, "directory should have 0700 permissions");
        }
        ...
        if let Some(ref path) = result.stdout.path {
            ...
            if p.exists() {
                ...
                assert_eq!(mode & 0o777, 0o600, "stdout file should have 0600 permissions");
            }
        }
        ...
        if let Some(ref path) = result.stdout.clean_path {
            ...
            if p.exists() {
                let mode = p.metadata().unwrap().permissions().mode();
                assert_eq!(mode & 0o777, 0o600, "clean file should have 0600 permissions");
            }
        }
    }
    ...
    fn test_strip_ansi_escapes_preserve_newlines() {
        let input = b"line1\nline2\r\nline3\ttab";
        let clean = strip_ansi_escapes(input);
        assert_eq!(String::from_utf8_lossy(&clean), "line1\nline2\r\nline3\ttab");
    }
    ...
    fn test_offload_clean_file_has_ansi_stripped() {
        ...
        if let Some(ref clean_path) = result.stdout.clean_path {
            ...
            assert!(!clean_content.contains("\x1b"), "clean file should not contain ANSI escapes");
            assert!(clean_content.contains("RED"), "clean file should contain visible text RED");
        }
    }
}
