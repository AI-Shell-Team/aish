...
impl PtyExecutor {
    ...
    fn execute_inner(
        ...
        let (stderr_pipe_read, stderr_pipe_write) = pipe().map_err(|e| {
            AishError::Pty(format!("failed to create stderr pipe: {e}"))
        })?;
    ...
    fn run_io_loop(
        ...
        while !child_exited {
            ...
            if select_result < 0 {
                ...
                if errno == libc::EINTR {
                    child_exited =
                        check_child_exit(child_pid, &mut exit_code);
                    continue;
                }
                return Err(AishError::Pty(format!(
                    "select error: errno {errno}"
                )));
            }
            ...
            if select_result == 0 {
                ...
                if !child_exited {
                    child_exited =
                        check_child_exit_nonblocking(child_pid, &mut exit_code);
                }
                continue;
            }
        ...
        let offload_result =
            offload.finalize(&stdout_tail, &stderr_tail, exit_code);
        ...
        let offload_value = if offload_result.stdout.path.is_some()
            || offload_result.stderr.path.is_some()
        {
            Some(
                serde_json::to_value(&offload_result)
...
fn child_main(
    ...
    let c_cmd = std::ffi::CString::new(command)
...
fn reap_child(pid: Pid, fallback_code: i32) -> (CommandStatus, i32) {
    match waitpid(pid, Some(nix::sys::wait::WaitPidFlag::WNOHANG)) {
        ...
        Ok(WaitStatus::Signaled(_, sig, _)) => {
            (CommandStatus::Error, 128 + sig as i32)
        }
        Ok(_) => {
