...
impl MemoryManager {
    ...
    pub fn new(memory_file: PathBuf) -> aish_core::Result<Self> {
        if !memory_file.exists() {
            ...
            let mut f = std::fs::File::create(&memory_file).map_err(|e| {
                AishError::Memory(format!("cannot create memory file: {}", e))
            })?;
            f.write_all(HEADER.as_bytes()).map_err(|e| {
                AishError::Memory(format!("cannot write memory header: {}", e))
            })?;
            return Ok(Self {
                memory_file,
                entries: Vec::new(),
    ...
    fn persist(&self) -> aish_core::Result<()> {
        ...
        std::fs::write(&self.memory_file, out).map_err(|e| {
            AishError::Memory(format!("failed to write memory file: {}", e))
        })
    }
}
...
fn parse_file(path: &Path) -> aish_core::Result<Vec<MemoryEntry>> {
    let content = std::fs::read_to_string(path).map_err(|e| {
        AishError::Memory(format!("cannot read memory file {:?}: {}", path, e))
    })?;
...
mod tests {
    ...
    fn test_roundtrip() {
        ...
        let id1 = mgr
            .store("I prefer dark theme", MemoryCategory::Preference, "auto", 1.0)
            .unwrap();
        let id2 = mgr
            .store("db port is 5432", MemoryCategory::Environment, "manual", 0.8)
            .unwrap();
    ...
    fn test_recall() {
        ...
        mgr.store("dark theme preference", MemoryCategory::Preference, "auto", 1.0)
            .unwrap();
        mgr.store("database port 5432", MemoryCategory::Environment, "manual", 0.8)
            .unwrap();
        mgr.store("use rust for systems code", MemoryCategory::Pattern, "auto", 0.6)
            .unwrap();
    ...
    fn test_duplicate_case_insensitive() {
        ...
        let mut mgr = MemoryManager::new(path).unwrap();
        let id1 = mgr
            .store("Database port 5432", MemoryCategory::Environment, "auto", 1.0)
            .unwrap();
        let id2 = mgr
            .store("database PORT 5432", MemoryCategory::Environment, "auto", 1.0)
            .unwrap();
    ...
    fn test_session_context_with_entries() {
        ...
        let mut mgr = MemoryManager::new(path).unwrap();
        mgr.store("test entry", MemoryCategory::Other, "test", 1.0).unwrap();
        let ctx = mgr.get_session_context();
        assert!(!ctx.is_empty());
