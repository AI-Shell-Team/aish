...
pub fn classify_input(input: &str) -> InputIntent {
    ...
    if trimmed.starts_with('/') && trimmed.split_whitespace().next().map_or(false, |cmd| {
        matches!(cmd, "/model" | "/setup" | "/plan" | "/token")
    }) {
...
mod tests {
    ...
    fn test_extract_ai_question() {
        assert_eq!(extract_ai_question("; how do I list files?"), "how do I list files?");
        assert_eq!(extract_ai_question("；你好"), "你好");
        assert_eq!(extract_ai_question("  ; spaced question  "), "spaced question");
    }
    ...
    fn test_classify_command_state_modify() {
        ...
        assert_eq!(classify_command("popd"), CommandCategory::BuiltinStateModify);
        assert_eq!(classify_command("dirs"), CommandCategory::BuiltinStateModify);
        assert_eq!(
