                    ...
                    match ch {
                        ...
                        'f' => {
                            return BuiltinResult::handled(
                                "export: -f: functions not supported",
                            )
                        }
                        _ => {
                            return BuiltinResult::handled(format!(
...
Any other input is executed as an external command via /bin/bash."#;
    ...
    fn handle_setup(&mut self, _args: &[&str]) -> BuiltinResult {
        ...
        if let Err(e) = std::fs::create_dir_all(&config_dir) {
            return BuiltinResult::handled(format!("setup: failed to create config directory: {}", e));
        }
        ...
        match wizard.run() {
            ...
            Err(e) => {
                BuiltinResult::handled(format!("setup cancelled or failed: {}", e))
            }
        }
    }
}
...
mod tests {
    ...
    fn test_shorten_path() {
        assert_eq!(shorten_path("/home/user/projects", "/home/user"), "~/projects");
    ...
    fn test_handle_exit_has_confirmation_message() {
        ...
        assert_eq!(
            result.output,
            Some("Exiting shell. Goodbye!".to_string())
        );
        assert!(!result.route_to_pty);
        assert_eq!(result.pty_command, None);
    }
