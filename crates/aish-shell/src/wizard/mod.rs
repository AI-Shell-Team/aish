...
pub fn get_all_providers() -> Vec<ProviderInfo> {
    vec![
        ...
        // 4. DeepSeek
        ProviderInfo::new("deepseek", "DeepSeek")
            .with_env_key("DEEPSEEK_API_KEY"),
        // 5. Gemini
        ProviderInfo::new("gemini", "Gemini")
            .with_env_key("GOOGLE_API_KEY"),
        // 6. Google
        ...
        ProviderInfo::new("minimax", "MiniMax")
            .with_env_key("MINIMAX_API_KEY"),
        // 9. Moonshot AI (multi-endpoint)
        ProviderInfo::new("moonshot", "Moonshot AI")
            .with_env_key("MOONSHOT_API_KEY"),
        ...
        ProviderInfo::new("bedrock", "Bedrock")
            .with_custom_api_base(),
        // 22. Custom (OpenAI-compatible)
        ProviderInfo::new("custom", "Custom")
            .with_custom_api_base(),
    ]
}
...
impl SetupWizard {
    ...
    fn select_entry_mode(&self) -> Result<String, AishError> {
        ...
        if free_key::has_free_key_module() {
            options.insert(0, DialogOption::new("free_key", t("cli.setup.action_use_free_key")));
        }
        ...
        loop {
            ...
            match free_key::register_free_key() {
                ...
                Err(e) => {
                    println!(
                        "  {}",
                        t("cli.setup.free_key_failed_with_reason").replace("{reason}", &e.to_string())
                    );
    ...
    fn select_provider(&mut self) -> Result<(), AishError> {
        ...
        let result = crate::tui::show_selection_dialog(
            ...
            &question,
            &options,
            true,  // allow custom provider (custom API base)
            true,  // allow cancel
        );
    ...
    fn select_endpoint(&self, eps: &[endpoints::EndpointInfo]) -> String {
        ...
        let question = t("cli.setup.provider_endpoint_header")
            .replace("{provider}", provider_label);

        let options: Vec<DialogOption> = eps
            .iter()
            .map(|e| {
                DialogOption::new(&e.api_base, format!("{}  [{}]", e.label, e.hint))
            })
            .collect();
    ...
    fn verify_and_save(&mut self) -> Result<(), AishError> {
        ...
        if !conn.ok {
            ...
            let reason = t("cli.setup.verify_simple_failed_with_reason")
                .replace("{reason}", err_msg);
            println!("\n  {}", reason);
        ...
        println!("  {}", t("cli.setup.connectivity_ok").replace("{}", &latency.to_string()));
        ...
        let full_reason = t("cli.setup.verify_simple_failed_with_reason")
        ...
        // Definitive failure
        return self.handle_tool_support_failure();
    }
    ...
    fn save_config(&self) -> Result<(), AishError> {
        ...
        println!("\n  {}", t("cli.setup.saved"));
        println!("  {}", t("cli.setup.config_path").replace("{}", &config_path.display().to_string()));
...
mod tests {
    ...
    fn test_provider_info_creation() {
        ...
        assert_eq!(provider.api_base, Some("https://api.example.com/v1".to_string()));
        assert_eq!(provider.requires_api_base, false);
