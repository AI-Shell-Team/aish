...
const COMMON_SHELL_COMMANDS: &[&str] = &[
    "alias", "bg", "bind", "builtin", "command", "compgen", "complete",
    "declare", "disown", "echo", "enable", "eval", "exec", "false", "fc",
    "fg", "getopts", "hash", "help", "jobs", "kill", "let", "local",
    "printf", "read", "readonly", "return", "set", "shift", "shopt",
    "source", "test", "times", "trap", "true", "type", "typeset", "ulimit",
    "umask", "unalias", "wait",
];
...
mod path_tests {
    ...
    fn test_common_commands_all_lowercase() {
        for cmd in COMMON_SHELL_COMMANDS {
            assert_eq!(*cmd, cmd.to_lowercase(), "COMMON_SHELL_COMMANDS must be lowercase: {}", cmd);
        }
    }
