...
const STOP_WORDS: &[&str] = &[
    ...
    "into", "through", "during", "from", "with", "for", "at", "by", "to",
    "of", "in", "on", "up", "out", "off", "over", "under", "again",
    "all", "each", "every", "both", "few", "more", "most", "other",
    "some", "such", "only", "own", "same", "also", "there", "here",
];
...
impl AiHandler {
    ...
    pub async fn diagnose_failure(
        ...
        let query = format!(
            ...
            command, exit_code,
            if output.len() > 4096 { &output[..output.floor_char_boundary(4096)] } else { output }
        );
    ...
    fn recall_memories(&mut self, query: &str) {
        ...
        if let Some(ref mut mm) = *guard {
            ...
            let text = if text.len() > budget {
                ...
                format!(
                    "{}\n</long-term-memory>",
                    truncated.rfind('\n').map(|i| &text[..i]).unwrap_or(truncated)
                )
            } else {
                text
            };
    ...
    fn error_correction_system_message(&mut self, command: &str, exit_code: i32, stderr: &str) -> Option<String> {
        let stderr_section = if stderr.is_empty() {
            String::new()
        } else {
            let preview = if stderr.len() > 2048 { &stderr[..floor_char_boundary(stderr, 2048)] } else { stderr };
            format!("\n**Command Output:**\n```\n{}\n```", preview)
        };
        let mut vars = HashMap::new();
        vars.insert("username".to_string(), whoami());
