...
fn find_fence_open(text: &str) -> Option<FenceOpen> {
    ...
    loop {
        ...
        return Some(FenceOpen {
            ...
            lang_end: line_end,
            code_start: if line_end < text.len() { line_end + 1 } else { text.len() },
        });
    }
}
...
fn render_table_to_segments(lines: &[&str], width: usize) -> Option<richrs::segment::Segments> {
    let headers = parse_pipe_row(lines.get(0)?)?;
...
fn render_code_block(console: &mut Console, lang: &str, code: &str, width: usize) {
    ...
    let syntax = Syntax::new(code_trimmed, lang)
            ...
            match seg {
                ...
                ContentSegment::Markdown(content) => {
                    ...
                    let md = Markdown::new(&content)
                        .inline_code_style(inline_style);
...
mod tests {
    ...
    fn test_parse_pipe_row() {
        assert_eq!(parse_pipe_row("| a | b | c |").unwrap(), vec!["a", "b", "c"]);
    }
    #[test]
