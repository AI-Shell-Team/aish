...
pub fn render_welcome(_version: &str, model: &str, skill_count: usize) -> String {
    ...
    for line in &content_lines {
        ...
        let padding = if inner_width > visible_len {
...
mod tests {
    ...
    fn test_render_prompt_with_home_substitution() {
        ...
        assert!(result.contains("~"), "should substitute home with ~: {}", result);
    ...
    fn test_render_prompt_success_symbol() {
        let result = render_prompt("/tmp", "test-model", 0, "aish");
        assert!(result.contains("\x1b[32m➜\x1b[0m"), "should have green single arrow on success");
        assert!(!result.contains("➜➜"), "should not have double arrow on success");
    }

    #[test]
    fn test_render_prompt_error_symbol() {
        let result = render_prompt("/tmp", "test-model", 1, "aish");
        assert!(result.contains("\x1b[31m➜➜\x1b[0m"), "should have red double arrow on error");
    }
    ...
    fn test_render_prompt_aish_mode_badge() {
        ...
        assert!(result.contains("\x1b[35m<aish>"), "aish badge should be magenta");
    }
    ...
    fn test_render_prompt_plan_mode_badge() {
        ...
        assert!(result.contains("\x1b[33m<plan>"), "plan badge should be yellow");
    }
    ...
    fn test_render_welcome_contains_logo() {
        ...
        assert!(result.contains("╰"), "should contain rounded box bottom-left");
        assert!(result.contains("╯"), "should contain rounded box bottom-right");
    ...
    fn test_render_welcome_contains_quick_start() {
        ...
        assert!(result.contains("ls, top, vim, ssh"), "should contain example commands");
    }
}
