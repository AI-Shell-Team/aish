...
fn run_inquire_selection(
    ...
    match item {
        Item::Real(i) => {
            Ok(DialogResult::Selected(options[*i].value.clone()))
        }
        Item::Custom => {
...
fn fallback_stdin_selection(
    ...
    if allow_custom {
        ...
        let matched = options.iter().find(|o| o.label.eq_ignore_ascii_case(&answer));
        match matched {
...
mod tests {
    ...
    fn test_dialog_result_equality() {
        ...
        assert_ne!(DialogResult::Selected("yes".to_string()), DialogResult::Cancelled);
        assert_ne!(
    ...
    fn test_dialog_option_structure() {
        ...
        let opt_with_desc = DialogOption::new("value2", "Label 2").with_description("A description");
