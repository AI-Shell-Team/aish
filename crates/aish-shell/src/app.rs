...
impl AishShell {
    ...
    pub fn new(config: ConfigModel) -> aish_core::Result<Self> {
        ...
        let security_manager =
            SecurityManager::from_config(None).unwrap_or_else(|_| {
                SecurityManager::new(SecurityPolicy::default_policy())
            });
        ...
        tool_registry.register(Box::new(
            aish_tools::SecureBashTool::with_security_check(security_check),
        ));
        ...
        let memory_manager: SharedMemoryManager =
            Arc::new(Mutex::new(MemoryManager::new(MemoryManager::default_path()).ok()));
        ...
        let skill_tool = {
            let skills_snapshot: std::collections::HashMap<String, aish_tools::SkillInfo> =
                skill_manager.list_skills().iter().map(|s| {
                    (s.metadata.name.clone(), aish_tools::SkillInfo {
            ...
            let lookup = Box::new(move |name: &str| {
                skills_snapshot.get(name).cloned()
            });
                ...
                match event.event_type {
                    ...
                    LlmEventType::ReasoningDelta => {
                        if let Some(delta) = event.data.get("delta").and_then(|d| d.as_str()) {
                            if !delta.is_empty() {
                                ...
                                let frame =
                                    reasoning_frame_ref.fetch_add(1, Ordering::SeqCst);
                                ...
                                let prev =
                                    reasoning_lines_displayed_ref.load(Ordering::SeqCst);
                    ...
                    LlmEventType::ToolExecutionStart => {
                        if let Some(name) = event.data.get("tool_name").and_then(|n| n.as_str()) {
                            animation_ref.stop();
                            clear_reasoning();
                            let args_preview = event.data.get("tool_args")
                            ...
                            println!("\x1b[36m{}: {} ({})\x1b[0m", t("shell.tool.prefix"), name, args_preview);
                            let _ = io::stdout().flush();
                        }
                    }
                    LlmEventType::ToolExecutionEnd => {
                        let is_diagnose = event.data.get("source")
                            .and_then(|s| s.as_str())
                            .map(|s| s == "system_diagnose_agent")
                            .unwrap_or(false);
                        if is_diagnose {
                            let name = event.data.get("tool_name")
                                .and_then(|n| n.as_str())
                                .unwrap_or("");
                            let ok = event.data.get("ok")
                    ...
                    LlmEventType::InteractionRequired => {
                        let prompt_text = event.data.get("prompt")
                            .and_then(|p| p.as_str())
                            .unwrap_or("");
        ...
        let confirmation_callback: Arc<dyn Fn(&str, &str) -> bool + Send + Sync> = Arc::new(
            |tool_name: &str, message: &str| {
                ...
                println!("\x1b[33m│\x1b[1;33m ⚠  Security Confirmation Required\x1b[0m{}", pad_to_width("", width.saturating_sub(38)));
                println!("\x1b[33m│\x1b[0m");
                println!("\x1b[33m│\x1b[0m  \x1b[1;36mTool:\x1b[0m   {}", tool_name);
                let reason_lines = wrap_text(message, width.saturating_sub(14));
                println!("\x1b[33m│\x1b[0m  \x1b[1;36mReason:\x1b[0m {}", reason_lines.lines().next().unwrap_or(""));
        ...
        print!("{}", prompt::render_welcome(&version, &config.model, skill_count));
        let _ = io::stdout().flush();

        // Initialize persistent PTY session
        let (cols, rows) = crossterm::terminal::size().unwrap_or((80, 24));
        let pty = aish_pty::PersistentPty::start(&state.cwd, rows, cols)
            .map_err(|e| aish_core::AishError::Pty(format!("failed to start persistent PTY: {e}")))?;
    ...
    pub fn run(&mut self) -> aish_core::Result<()> {
        ...
        loop {
            ...
            let prompt_str = prompt::render_prompt(&self.state.cwd, &self.config.model, self.state.last_exit_code, mode);
                    ...
                    match result {
                        Ok(response) => {
                            ...
                            if plan_state.phase == aish_core::PlanPhase::Normal
                                ...
                                && plan_state.artifact_path.is_some()
                            {
                                // Plan was exited — read artifact and present for approval
                                let artifact_text = aish_core::plan::read_artifact_text(
                                    plan_state.artifact_path.as_ref().unwrap(),
                                );

                                // Use the enhanced plan approval flow
                                use crate::wizard::plan_approval::{PlanApprovalDecision, PlanApprovalFlow};
                                ...
                                match decision {
                                    PlanApprovalDecision::Approved => {
                                        ...
                                        if let Ok(_snapshot) = aish_core::plan::create_approved_snapshot(&mut state) {
                                            println!("\x1b[32m\u{2713} Plan approved. Implementation tools are now available.\x1b[0m");
                                            println!("\x1b[2m  Snapshot saved at: {}\x1b[0m",
                                                state.approved_artifact_path.as_deref().unwrap_or("(none)"));
                                        }
                                    }
                                    PlanApprovalDecision::ChangesRequested { feedback } => {
                                        ...
                                        {
                                            let plan_state_lock = self.ai_handler.plan_state_ptr();
                                            let mut ps = plan_state_lock.lock().unwrap();
                                            ps.approval_status = aish_core::PlanApprovalStatus::ChangesRequested;
                                            ps.approval_feedback_summary = if feedback.is_empty() {
                                                None
                                            } else {
                                                Some(feedback.clone())
                                            };
    ...
    fn handle_plan_command(&mut self, parts: &[&str]) {
        ...
        if !subcommand.is_empty()
            && !["start", "status", "exit"].contains(&subcommand)
        {
            ...
            PlanPhase::Normal => {
                match subcommand {
                    ...
                    _ => {
                        ...
                        println!("\x1b[2mType ; followed by your planning request to start.\x1b[0m");
                    }
                }
            }
        }
    ...
    fn sync_command_to_pty(&mut self, command: &str) {
        ...
        let _ = self.pty.execute_command(command, std::time::Duration::from_secs(5));
    }
    ...
    pub fn remember_approved_command(&mut self, command: &str) {
        ...
        if !self.config.approved_ai_commands.contains(&command.to_string()) {
    ...
    fn execute_script(&mut self, input: &str) -> i32 {
        ...
        let script = match aish_scripts::loader::parse_script_file(std::path::Path::new(script_path)) {
            Ok(s) => s,
            Err(e) => {
    ...
    if tool_name == "write_file" {
        if let Some(obj) = args.as_object() {
            let display: serde_json::Map<String, serde_json::Value> = obj
                ...
                .collect();
            return truncate_str(
                &serde_json::Value::Object(display).to_string(),
                120,
            );
        }
    }
