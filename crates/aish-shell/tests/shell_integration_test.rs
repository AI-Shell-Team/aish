...
use aish_shell::input::{
    classify_input, classify_command, extract_ai_question,
};
use aish_shell::types::{InputIntent, CommandCategory};
...
fn test_classify_command_external() {
    ...
    assert_eq!(classify_command("python script.py"), CommandCategory::External);
...
fn test_classify_command_with_extra_whitespace() {
    ...
    assert_eq!(
        classify_command("\tsudo ls"),
        CommandCategory::PtyRequired
    );
    assert_eq!(
        classify_command("  exit  "),
        CommandCategory::Rejected
    );
}
...
fn test_classify_input_special_command() {
    ...
    assert_eq!(classify_input("  /model claude-3"), InputIntent::SpecialCommand);
}
...
fn test_classify_input_builtin_command() {
    ...
    assert_eq!(classify_input("export FOO=bar"), InputIntent::BuiltinCommand);
...
fn test_classify_input_script_call() {
    ...
    assert_eq!(classify_input("/home/user/scripts/test.aish"), InputIntent::ScriptCall);
}
...
fn test_classify_input_external_command() {
    ...
    assert_eq!(classify_input("cargo build --release"), InputIntent::Command);
}
...
fn test_extract_ai_question() {
    ...
    assert_eq!(
        extract_ai_question("hello world"),
        "hello world"
    );
}
