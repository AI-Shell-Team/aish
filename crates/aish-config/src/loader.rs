...
impl ConfigLoader {
    ...
    pub fn load(config_path: Option<&Path>) -> Result<ConfigModel> {
        ...
        let mut config = if path.exists() {
            ...
            serde_yaml::from_str(&raw)
                .map_err(|e| AishError::Config(format!("failed to parse {}: {e}", path.display())))?
        } else {
    ...
    pub fn save(config: &ConfigModel, path: &Path) -> Result<()> {
        ...
        std::fs::write(path, yaml).map_err(|e| {
            AishError::Config(format!("failed to write {}: {e}", path.display()))
        })?;
...
mod tests {
    ...
    fn test_apply_env_overrides_codex_auth_path() {
        ...
        assert_eq!(config.codex_auth_path.as_deref(), Some("/tmp/test_auth.json"));
