...
/// Immutable snapshot of rotation state for UI display (`/usage`).
...
impl RotationState {
    ...
    pub fn snapshot(&self) -> RotationSnapshot {
        ...
        let account_names: Vec<String> =
            self.enabled_accounts().map(|(_, a)| a.name.clone()).collect();
...
mod tests {
    ...
    fn is_active_requires_multiple_accounts_or_fallbacks() {
        ...
        let with_fb = RotationState::new("m".into(), accounts(1), vec!["x".into()], RetryPolicy::default());
