...
impl StreamParser {
    ...
    pub fn parse_response(response: &serde_json::Value) -> (Option<String>, Vec<ToolCall>, Option<TokenUsage>) {
        ...
        if let Some(choices) = choices {
            if let Some(choice) = choices.first() {
                ...
                return (content, tool_calls, if has_usage { Some(usage) } else { None });
            }
        }
        (None, Vec::new(), None)
    }
