...
pub fn login_with_browser(
    ...
    let code = parse_callback_query_param(&request, "code")
        .ok_or_else(|| {
...
pub fn login_with_device_code(provider: &OAuthProviderSpec) -> Result<OAuthTokens, String> {
    ...
    let resp: HashMap<String, serde_json::Value> = client
        .post(device_url)
        .form(&[("client_id", provider.client_id.as_str()), ("scope", provider.scope.as_str())])
        .send()
...
pub fn exchange_code_for_tokens(
    ...
    if let Some(error) = resp.get("error").and_then(|v: &serde_json::Value| v.as_str()) {
        let desc = resp
            .get("error_description")
...
fn parse_token_response(
    ...
    Ok(OAuthTokens {
        access_token,
        refresh_token: resp.get("refresh_token").and_then(|v| v.as_str()).map(String::from),
        id_token: resp.get("id_token").and_then(|v| v.as_str()).map(String::from),
        token_type: resp
            .get("token_type")
            .and_then(|v| v.as_str())
            .unwrap_or("Bearer")
...
mod tests {
    ...
    fn test_generate_pkce() {
        ...
        assert!(!pkce.code_challenge.is_empty(), "challenge must not be empty");
        assert_ne!(
