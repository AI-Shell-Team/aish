...
impl LlmSession {
    ...
    pub fn set_confirmation_callback(
        &mut self,
        cb: Arc<dyn Fn(&str, &str) -> bool + Send + Sync>,
    ) {
        ...
        loop {
            ...
            if iterations >= max_iterations {
                ...
                return Err(AishError::Llm(
                    "Max tool call iterations reached".into(),
                ));
            }
            iterations += 1;
                                            ...
                                            match event {
                                                ...
                                                SseEvent::ReasoningDelta(delta) => {
                                                    if !reasoning_started {
                                                        ...
                                                        self.emit_event(LlmEvent {
                                                            event_type: LlmEventType::ReasoningStart,
                                                            data: serde_json::json!({}),
                                                            timestamp: now_timestamp(),
                                                            metadata: None,
                                                        });
    ...
    pub fn create_diagnose_subsession(&self, tools: Vec<Box<dyn Tool>>) -> crate::subsession::SubSession {
...
fn trim_messages(messages: Vec<ChatMessage>, max_tokens: usize, preserve_recent: usize) -> Vec<ChatMessage> {
...
mod tests {
    ...
    fn test_trim_messages_preserves_system() {
        ...
        for i in 0..20 {
            msgs.push(make_msg("user", &format!("message {} with some content to make it longer", i)));
        }
    ...
    fn test_trim_messages_preserves_recent() {
        ...
        for i in 0..20 {
            msgs.push(make_msg("user", &format!("message number {} with padding content here", i)));
        }
        let result = trim_messages(msgs, 50, 3);
        // Last 3 messages should be preserved
        assert_eq!(result.last().unwrap().content.as_deref(), Some("message number 19 with padding content here"));
    }
    ...
    fn test_session_update_model() {
        ...
        assert!(true); // compile-time check that method exists
    }
    ...
    fn test_subsession_independent_plan_state() {
        ...
        assert_eq!(sub.plan_state().lock().unwrap().phase, aish_core::PlanPhase::Normal);

        // Parent should still be in Planning mode
        assert_eq!(session.plan_state().lock().unwrap().phase, aish_core::PlanPhase::Planning);
    }
