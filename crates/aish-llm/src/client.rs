...
impl LiteLLMClient {
    ...
    pub async fn test_connection(&self) -> Result<(), String> {
        ...
        match result {
            ...
            Err(e) => {
                ...
                if err_msg.contains("401") || err_msg.contains("403") || err_msg.contains("unauthorized") {
                    Ok(())
                } else {
    ...
    fn convert_message(msg: &ChatMessage) -> litellm_rs::Message {
        match msg.role.as_str() {
            "system" => litellm_rs::system_message(
                msg.content.as_deref().unwrap_or("")
            ),
            "user" => litellm_rs::user_message(
                msg.content.as_deref().unwrap_or("")
            ),
            "assistant" => litellm_rs::assistant_message(
                msg.content.as_deref().unwrap_or("")
            ),
            _ => litellm_rs::user_message(
                msg.content.as_deref().unwrap_or("")
            ),
        }
    }
    ...
    pub async fn chat_completion(
        ...
        if tools.is_some() && tools.unwrap().len() > 0 {
            return self.fallback_chat_completion(messages, tools, false, temperature, max_tokens).await;
        }

        // Convert messages to litellm_rs format
        let litellm_messages: Vec<litellm_rs::Message> = messages
            .iter()
    ...
    pub async fn chat_completion_stream(
        ...
        self.fallback_chat_completion(messages, tools, true, temperature, max_tokens).await
    }
    ...
    async fn fallback_chat_completion(
        ...
        let api_base = self.api_base.as_deref().unwrap_or("https://api.openai.com/v1");

        // Determine API key - try environment variable if not provided
        let api_key = self.api_key.clone().unwrap_or_else(|| {
            std::env::var("OPENAI_API_KEY")
                .expect("No API key provided and OPENAI_API_KEY not set")
        });
    ...
    pub fn new_with_retry(
        ...
        for attempt in 0..=max_retries {
            ...
            match rt.block_on(client.test_connection()) {
                Ok(()) => {
                    if attempt > 0 {
                        tracing::info!("LLM connectivity check succeeded on attempt {}", attempt + 1);
                    }
                    return client;
                }
                Err(e) => {
                    tracing::warn!("LLM connectivity check attempt {} failed: {}", attempt + 1, e);
                }
            }
        }
...
mod tests {
    ...
    fn test_new_with_retry_returns_client() {
        ...
        let client = LlmClient::new_with_retry(
            "http://127.0.0.1:1/v1",
            "sk-test",
            "gpt-4o",
            1,
    ...
    fn test_litellm_client_construction() {
        let client = LiteLLMClient::new(Some("https://api.openai.com/v1"), Some("sk-test"), "gpt-4o");
        assert_eq!(client.model_name(), "gpt-4o");
    ...
    fn test_format_http_error_429() {
        let msg = format_http_error(reqwest::StatusCode::TOO_MANY_REQUESTS, "Rate limit exceeded");
