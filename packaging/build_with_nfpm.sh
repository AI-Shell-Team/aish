...
if ! command -v nfpm &> /dev/null; then
  ...
  echo -e "     pip install nfpm          # Python wrapper (ships the Go binary)"
  echo -e "     brew install nfpm         # macOS Homebrew"
...
build_package() {
  ...
  # Build with nfpm (-p sets the packager format, -t sets the target output directory)
  if nfpm pkg -f nfpm.yaml -p "$pkg_type" -t "$OUTPUT_DIR"; then
